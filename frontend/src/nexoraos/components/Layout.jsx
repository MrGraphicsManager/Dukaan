...
import { LayoutDashboard, ShoppingBag, Grid3x3, ClipboardList, ChefHat, Utensils, Package, Users, Shield, BarChart3, CreditCard, Settings as SettingsIcon, LogOut, Coffee, ChevronDown, Plus, Check, Building2, Menu, X, ArrowLeft } from "lucide-react";

const NAV = [
  { to: "/nexoraos/dashboard", icon: LayoutDashboard, label: "Dashboard", testid: "sidebar-nav-dashboard" },
  { to: "/nexoraos/pos", icon: ShoppingBag, label: "POS", testid: "sidebar-nav-pos", roles: ["owner","manager","cashier"] },
  { to: "/nexoraos/tables", icon: Grid3x3, label: "Tables", testid: "sidebar-nav-tables", roles: ["owner","manager","cashier"] },
  { to: "/nexoraos/orders", icon: ClipboardList, label: "Orders", testid: "sidebar-nav-orders" },
  { to: "/nexoraos/kitchen", icon: ChefHat, label: "Kitchen", testid: "sidebar-nav-kitchen" },
  { to: "/nexoraos/menu", icon: Utensils, label: "Menu", testid: "sidebar-nav-menu", roles: ["owner","manager"] },
  { to: "/nexoraos/inventory", icon: Package, label: "Inventory", testid: "sidebar-nav-inventory", roles: ["owner","manager"] },
  { to: "/nexoraos/customers", icon: Users, label: "Customers", testid: "sidebar-nav-customers", roles: ["owner","manager","cashier"] },
  { to: "/nexoraos/staff", icon: Shield, label: "Staff", testid: "sidebar-nav-staff", roles: ["owner","manager"] },
  { to: "/nexoraos/reports", icon: BarChart3, label: "Reports", testid: "sidebar-nav-reports", roles: ["owner","manager"] },
];
const BOTTOM = [
  { to: "/nexoraos/subscription", icon: CreditCard, label: "Subscription", testid: "sidebar-nav-subscription", roles: ["owner"] },
  { to: "/nexoraos/settings", icon: SettingsIcon, label: "Settings", testid: "sidebar-nav-settings" },
];
// Mobile bottom nav — most-used routes
const MOBILE_TABS = [
  { to: "/nexoraos/dashboard", icon: LayoutDashboard, label: "Home" },
  { to: "/nexoraos/pos", icon: ShoppingBag, label: "POS", roles: ["owner","manager","cashier"] },
  { to: "/nexoraos/orders", icon: ClipboardList, label: "Orders" },
  { to: "/nexoraos/kitchen", icon: ChefHat, label: "KDS" },
];
    ...
    <>
      ...
      <div className="p-3 border-t border-[#F2E8DC]">
        <div className="flex items-center gap-3 px-2 py-2">
          ...
          <button data-testid="sidebar-logout-button" onClick={() => { logout(); nav("/nexoraos/login"); }} className="p-2 rounded-lg hover:bg-[#F5ECE1] text-[#6B5A52]" title="Logout"><LogOut className="w-4 h-4" /></button>
        </div>
        ...
        <div className="absolute left-3 right-3 top-[calc(100%-4px)] card p-1.5 z-30 shadow-lg">
          ...
          <div className="mt-1 border-t border-[#F2E8DC] pt-1">
            {canAdd ? (
              ...
              <div className="px-2.5 py-2 text-[11px] text-[#9C8A80]">Upgrade to <Link to="/nexoraos/subscription" className="text-[#C85A32] font-semibold">Pro Plan</Link> to add more cafés.</div>
            ) : null}
          </div>
        </div>
      )}
