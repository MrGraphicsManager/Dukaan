...
export default function Kitchen() {
  ...
  React.useEffect(() => {
    ...
    if (!token) return;
    const rawBase = (process.env.REACT_APP_NEXORA_API_URL || process.env.REACT_APP_BACKEND_URL || (typeof window !== "undefined" ? window.location.origin : "")).replace(/\/+$/, "");
    const base = rawBase.replace(/^http/, "ws");
