            ...
            {shown.length===0 ? <tr><td colSpan={8} className="text-center py-16 text-[#9E8E81]">No cafés yet</td></tr> :
              shown.map(c => (
                <tr key={c.id} className="border-t border-[#3D312A] hover:bg-[#2E2521]">
                  ...
                  <td className="px-4 py-3 text-right"><Link to={`/nexoraos/admin/cafes/${c.id}`} data-testid={`admin-cafe-view-${c.id}`} className="text-[#E8C8B5]"><ChevronRight className="w-4 h-4"/></Link></td>
                </tr>
              ))}
          </tbody>
        </table>
