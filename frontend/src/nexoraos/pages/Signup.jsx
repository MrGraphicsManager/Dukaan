...
export default function Signup() {
  ...
  const verify = async (e) => {
    ...
    try { const { data } = await api.post("/auth/verify-otp", { email: form.email, otp }); await setTokenAndUser(data.token);
      toast.success("Account created 🎉"); nav("/nexoraos/dashboard"); }
  ...
  const googleOk = async (resp) => {
    try {
      ...
      toast.success("Welcome to NexoraOS 🎉"); nav("/nexoraos/dashboard");
    ...
    <AuthShell title={step===1 ? "Create your café" : "Verify your email"} subtitle={step===1 ? "Start with a 14-day free trial." : `We sent a 6-digit code to ${form.email}`}>
      {step === 1 ? (
        <>
          ...
          <div className="text-sm text-[#6B5A52] text-center mt-5">Already have an account? <Link to="/nexoraos/login" className="text-[#C85A32] font-semibold hover:underline">Sign in</Link></div>
        </>
      ) : (
